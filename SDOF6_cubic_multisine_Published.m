function SDOF6_cubic_multisine_Published
clc;
clear all;
...
Frms = 1*sqrt(2); Nsample =2^13;Fs = 30;%Fs:sampling frequency
dt = 1/Fs;
t = (0:dt:(Nsample-1)*dt); %time
df = Fs/Nsample;f = (df:df:Fs); %frequency

Deltaf = Fs/Nsample;
fmax = 6; %maximum frequency of multisine
fmin = 0; %minimum frequency of multisine
...
%%%% cpsd
[PFx f1] = cpsd(F,x(:,1),win,overlap,length(win),Fs);
[PxF f1] = cpsd(x(:,1),F,win,overlap,length(win),Fs);
[Pxx f1] = cpsd(x(:,1),x(:,1),win,overlap,length(win),Fs);
